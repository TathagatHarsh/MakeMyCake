...
import { phiOf, surfaceRadius, type Sector, type TierDims } from "./geometry";
import type { PlaqueFootprint } from "./MessagePlaque";
import { toppingGeo } from "./toppingGeometry";
  ...
  switch (spec.placement) {
    case "top-ring": {
      ...
      for (let i = 0; i < n; i++) {
        ...
        drop(Math.cos(a) * r, topY + size * 0.4, Math.sin(a) * r);
      }
      break;
    }
    case "base-border": {
      const r = surfaceRadius(config.shape, bottom.radius) + size * 0.55;
      for (let i = 0; i < n; i++) {
        const a = (i / n) * Math.PI * 2 + (rng() - 0.5) * (Math.PI / n) * 0.6;
        drop(Math.cos(a) * r, size * 0.42, Math.sin(a) * r);
      }
      break;
    }
    case "crown": {
      ...
      for (let ring = 0; ring < rings && placed < n; ring++) {
        ...
        for (let i = 0; i < count && placed < n; i++) {
          ...
          drop(Math.cos(a) * rr, topY + size * (0.4 + ring * 0.7), Math.sin(a) * rr);
          placed++;
        }
      }
      break;
    ...
    case "cascade": {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const tier = tiers.reduce((acc, d) => (y >= d.y ? d : acc), tiers[0]);
        const r = surfaceRadius(config.shape, tier.radius) + size * 0.45;
        drop(Math.cos(a) * r, Math.max(size * 0.4, y), Math.sin(a) * r, 0.32);
      }
      break;
    }
    default: {
      ...
      for (const [x, z] of pts) drop(x, topY + size * 0.4, z);
    }
  }
...
function ToppingLayer({
  ...
  const instances = useMemo(
    () => place(
      ...
      geo.flat, maxInstances, keepOff, sector,
    ),
    [spec, config, tiers, seed, geo.scale, geo.flat, maxInstances, keepOff, sector],
  );
